...
- Use `classifier` from `cca.py` to run CCA with the same reference design
- Parse Start1..Start24 / Stop1..Stop24 markers into trials
- For each trial, cut a segment (default 4s) and classify
...
def _extract_trials_from_markers(
    ...
    for idx, (lab, ts) in enumerate(zip(labels, marker_ts)):
    ...
    for i in range(n_trials):
        s_num, s_label, s_time = start_events[i]
        t_num, t_label, t_time = stop_events[i]
        trial_idx = i + 1
        trials.append(
            {
                "trial_idx": trial_idx,
...
#     """
#     Extract a segment for one trial.

#     Returns (segment, used_duration_seconds).
#     Segment shape: (n_samples, n_channels) or (None, 0) if invalid.
#     """
#     if eeg_ts is None or len(eeg_ts) == 0:
#         return None, 0.0

#     # Convert times to sample indices
#     start_idx = int(np.searchsorted(eeg_ts, start_time, side="left"))
#     stop_idx = int(np.searchsorted(eeg_ts, stop_time, side="right"))

#     start_idx = max(0, min(start_idx, len(eeg_ts) - 1))
#     stop_idx = max(start_idx + 1, min(stop_idx, len(eeg_ts)))

#     available_len = stop_idx - start_idx
#     if available_len <= 0:
#         return None, 0.0

#     target_len = int(duration * srate)
#     used_len = min(available_len, target_len)
#     end_idx = start_idx + used_len
...
def run_cca_offline(
    ...
    """
    Run offline CCA on all provided XDF files.
...
def _plot_accuracy_by_freq(df: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.ylabel("Accuracy")
    plt.title("CCA Offline Accuracy by Frequency")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run offline CCA analysis on XDF files using xdf_loader and cca.classifier",
    )
    parser.add_argument(
        "--raw-dir",
        type=str,
    ...
    df_results = run_cca_offline(xdf_paths, duration=args.duration, display="mobile")
    if df_results.empty:
    ...
    csv_path = results_dir / "cca_offline_results.csv"
    ...
    acc_fig_path = results_dir / "cca_offline_accuracy_by_freq.png"
