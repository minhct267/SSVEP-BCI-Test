"""
CCA Offline Analysis with Robust Detrending
===========================================

This script mirrors `cca_offline.py` but replaces the initial bandpass + DC
removal in `load_xdf_eeg` by a robust detrending step from `meegkit.detrend`.

Pipeline:
- Load raw EEG + markers using `load_xdf_eeg` with `apply_filter=False`
- Apply `meegkit.detrend.detrend` once on the continuous EEG to remove slow
  trends and large-amplitude drifts (robustly)
...
- Aggregate results into a CSV and a summary figure, with `_db` suffix
"""
...
def run_cca_offline_rd(
    xdf_paths: Iterable[Path],
    duration: float = 4.0,
    display: str = "mobile",
) -> pd.DataFrame:
    """
    Run offline CCA with robust detrending on all provided XDF files.
    ...
    for xdf_path in xdf_paths:
        ...
        print(f"\nProcessing {xdf_path} (subject={subject_id}) [robust_detrend]")
        ...
        try:
            eeg_data_rd, weights, basis_mat = robust_detrend(
                eeg_data,
                order=3,
                basis="polynomials",
                threshold=3,
            ...
            records.append(
                {
                    ...
                    "preproc": "robust_detrend",
                }
            )

    if not records:
        print("No valid trials found across all XDF files (robust_detrend).")
        return pd.DataFrame()
...
def _plot_accuracy_by_freq(df: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.ylabel("Accuracy")
    plt.title("CCA Offline Accuracy by Frequency (Robust Detrend)")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run offline CCA analysis on XDF files using robust detrending "
            "from meegkit.detrend and cca.classifier"
        ),
    )
    parser.add_argument(
        "--raw-dir",
    ...
    df_results = run_cca_offline_rd(xdf_paths, duration=args.duration, display="mobile")
    if df_results.empty:
    ...
    csv_path = results_dir / "cca_offline_results_db.csv"
    ...
    acc_fig_path = results_dir / "cca_offline_accuracy_by_freq_db.png"
