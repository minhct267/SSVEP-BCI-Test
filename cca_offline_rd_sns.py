"""
CCA Offline Analysis with Robust Detrending + SNS + Bandpass
============================================================

This script combines robust detrending from `meegkit.detrend` and Sensor Noise
Suppression (SNS) from `meegkit.sns`, followed by a bandpass filter, before
running CCA as in `cca_offline_rd.py` / `cca_offline.py`.

Pipeline:
- Load raw EEG + markers using `load_xdf_eeg` with `apply_filter=False`
- Apply robust detrending (polynomial order=3) on the continuous EEG
- Apply SNS (n_neighbors=7, skip=0) on the detrended EEG
- Apply bandpass 2–45 Hz (Butterworth, order=3) using `bandpass_filter_eeg`
...
- Aggregate results into a CSV and a summary figure, with `_rd_sns` suffix
"""
...
from xdf_loader import bandpass_filter_eeg, load_xdf_eeg
...
def run_cca_offline_rd_sns(
    xdf_paths: Iterable[Path],
    duration: float = 4.0,
    display: str = "mobile",
) -> pd.DataFrame:
    """
    Run offline CCA with robust detrending + SNS + bandpass on all provided XDF files.
    ...
    for xdf_path in xdf_paths:
        ...
        print(f"\nProcessing {xdf_path} (subject={subject_id}) [robust_detrend+sns+bandpass]")

        try:
            eeg_data, srate, ch_labels, eeg_ts, markers, marker_ts, eeg_info = load_xdf_eeg(
                xdf_path, apply_filter=False
            )
        ...
        # Robust detrending on the continuous EEG (time x channels)
        try:
            ...
            print(f"  [WARNING] Robust detrend failed for {xdf_path}: {e}")
            continue
        ...
        # Apply bandpass filter 2–45 Hz after SNS
        try:
            eeg_final = bandpass_filter_eeg(eeg_rd_sns, srate, low=2.0, high=45.0, order=3)
        except Exception as e:
            print(f"  [WARNING] Bandpass after SNS failed for {xdf_path}: {e}")
            continue
            ...
            records.append(
                {
                    ...
                    "preproc": "robust_detrend+sns+bandpass",
                }
            )

    if not records:
        print("No valid trials found across all XDF files (robust_detrend+sns+bandpass).")
        return pd.DataFrame()
...
def _plot_accuracy_by_freq(df: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.ylabel("Accuracy")
    plt.title("CCA Offline Accuracy by Frequency (Robust Detrend + SNS)")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run offline CCA analysis on XDF files using robust detrending and SNS "
            "from meegkit, bandpass filtering, and cca.classifier"
        ),
    )
    parser.add_argument(
        "--raw-dir",
    ...
    df_results = run_cca_offline_rd_sns(xdf_paths, duration=args.duration, display="mobile")
    if df_results.empty:
    ...
    csv_path = results_dir / "cca_offline_results_rd_sns.csv"
    ...
    acc_fig_path = results_dir / "cca_offline_accuracy_by_freq_rd_sns.png"
